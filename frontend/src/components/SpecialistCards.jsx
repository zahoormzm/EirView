    ...
    <section className={compact ? '' : 'mt-6'}>
      ...
      <div className={`grid grid-cols-1 ${compact ? 'gap-3' : 'md:grid-cols-2 gap-4'}`}>
        {visibleItems.map((item) => (
          <div key={item.specialist} className={`rounded-xl border ${compact ? 'p-3' : 'p-4'} ${item.urgency === 'urgent' ? 'border-red-500' : 'border-amber-500'}`}>
            ...
            {!!item.metrics?.length && (
              <div className={`${compact ? 'mt-3 space-y-1.5' : 'mt-4 space-y-2'}`}>
                {(compact ? item.metrics.slice(0, 1) : item.metrics).map((metric) => (
                  <div key={`${item.specialist}-${metric.metric}-${metric.reason}`} className={`rounded-lg bg-slate-50 border border-slate-200 ${compact ? 'px-2.5 py-2' : 'px-3 py-2'}`}>
