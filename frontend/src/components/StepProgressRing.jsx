export default function StepProgressRing({ current = 0, goal = 7500, size = 160 }) {
  const strokeWidth = 8;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col items-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-[220px]">
        ...
        <text x={center} y={center} textAnchor="middle" className="fill-slate-900 text-[20px] font-mono font-medium">{current.toLocaleString()}</text>
        <text x={center} y={center + 18} textAnchor="middle" className="fill-slate-400 text-[12px]">/ {goal.toLocaleString()}</text>
      </svg>
      <div className="text-sm text-slate-500 text-center mt-3">steps today</div>
    </div>
  );
}
