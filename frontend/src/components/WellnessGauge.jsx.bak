export default function WellnessGauge({ score = 0, breakdown = [] }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <svg viewBox="0 0 200 160" className="w-full max-w-[260px] mx-auto">
        ...
        />
        <text x="100" y="88" textAnchor="middle" className="fill-slate-900 text-3xl font-bold font-mono">{clamped}</text>
      ...
      <div className="mt-6">
        {breakdown.length ? breakdown.map((item) => (
          <div key={item.name} className="flex justify-between py-1.5 border-b border-slate-100">
            ...
            <span className="text-sm font-mono text-red-500">-{item.penalty}</span>
          </div>
