          ...
          {data.map((row, index) => {
            ...
            return (
              <Fragment key={row.id || index}>
                ...
                {open === index && (
                  <tr>
                    <td colSpan="5" className="bg-slate-50 p-3 rounded text-xs font-mono overflow-x-auto whitespace-pre-wrap">{JSON.stringify({ input: row.tool_input, output: row.tool_output, response: row.response }, null, 2)}</td>
                  </tr>
                )}
              </Fragment>
            );
