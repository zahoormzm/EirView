...
export default function MetricCard({ label, value, unit, delta, icon: Icon, status = 'neutral' }) {
  const statusClass = {
    good: 'bg-green-500',
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
      ...
      <div className="text-2xl font-mono font-medium tabular-nums text-slate-900 mt-1">{display}</div>
