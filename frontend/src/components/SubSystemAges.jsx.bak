...
export default function SubSystemAges({ data }) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="h-3 bg-slate-100 rounded-full">
                  <div className={`h-3 rounded-full ${color}`} style={{ width: `${(value / maxScale) * 100}%` }} />
                </div>
              </div>
              <span className="font-mono text-sm w-16 text-right">{value.toFixed(1)}</span>
            </div>
            <div className={`text-xs mt-1 ml-29 ${delta < 0 ? 'text-emerald-600' : delta > 0 ? 'text-red-600' : 'text-slate-500'}`}>
