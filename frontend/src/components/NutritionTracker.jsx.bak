...
export default function NutritionTracker({ target }) {
  const { selectedUserId, dashboard, showToast, setDashboard } = useStore();
  const [custom, setCustom] = useState('');
  const current = dashboard?.water_today_ml || 0;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <div className="flex gap-2 mt-3 flex-wrap">
        <button onClick={() => addWater(250)} className="border border-slate-300 text-slate-700 rounded-lg px-4 py-2 hover:bg-slate-50">+250ml</button>
        <button onClick={() => addWater(500)} className="border border-slate-300 text-slate-700 rounded-lg px-4 py-2 hover:bg-slate-50">+500ml</button>
        <div className="flex gap-2">
          <input value={custom} onChange={(event) => setCustom(event.target.value)} placeholder="Custom" className="border border-slate-300 rounded-lg px-3 py-2 text-sm w-24" />
          <button onClick={() => { if (custom) addWater(Number(custom)); setCustom(''); }} className="border border-slate-300 text-slate-700 rounded-lg px-4 py-2 hover:bg-slate-50">Add</button>
        </div>
      </div>
    </div>
  );
