...
from backend.formulas import calculate_bio_age, mental_wellness_score, nutrition_targets, project_risk, simulate_habit_change
...
async def get_transparency(user_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        global_cursor = await db.execute("SELECT * FROM agent_logs ORDER BY timestamp DESC LIMIT 100")
        global_rows = await global_cursor.fetchall()
        global_logs = [dict(row) for row in global_rows]
        ...
        return {
            "agent_logs": logs,
            "global_agent_logs": global_logs,
            "selected_user_has_activity": bool(logs),
            "model_stats": model_stats,
            "exact_model_stats": exact_model_stats,
            "recent_calls": ai_router.call_log[-50:],
            "provider_status": {
                "claude_configured": bool(getattr(ai_router, "anthropic", None)),
                "gemini_configured": bool(getattr(ai_router, "gemini", None)),
            },
        }
    finally:
        await db.close()
