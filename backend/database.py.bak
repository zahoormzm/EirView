...
PROFILE_COLUMNS: list[str] = [
    ...
    "temperature_c", "humidity_pct", "aqi", "uv_index", "phq9_score", "stress_level",
    ...
    "emergency_contact_name", "emergency_contact_phone", "bio_age_overall", "bio_age_cardiovascular",
...
async def get_water_today(user_id: str) -> int:
    ...
    try:
        row = await (
            await db.execute("SELECT COALESCE(SUM(amount_ml),0) AS total FROM water_log WHERE user_id=? AND DATE(timestamp)=DATE('now')", (user_id,))
        ).fetchone()
...
async def log_agent_action(user_id: str, agent_name: str, action: str, **kwargs: Any) -> None:
    ...
    try:
        await db.execute(
            ...
            (user_id, agent_name, action, tool_name, tool_input, tool_output, prompt, response, tokens_in, tokens_out, latency_ms, model)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
            """,
            (
                user_id,
